  ...
  <main class="page">
    ...
    <section class="list-section">
      <h2 class="section-title">
        Restaurants
        <span v-if="restaurantList.length" class="count">{{ restaurantList.length }}</span>
      </h2>

      <p v-if="!restaurantList.length" class="empty-state">
      ...
      <TransitionGroup name="list" tag="ul" class="item-list">
        <li v-for="(restaurant, index) in restaurantList" :key="restaurant.name" class="item-card">
          ...
          <button class="btn-remove" @click="removeRestaurant(index)" aria-label="Remove">
            &times;
          </button>
        </li>
      </TransitionGroup>
